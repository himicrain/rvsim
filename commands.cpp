...
bool command_match_hex_number(string& command, unsigned int& i, unsigned int& num) {
  unsigned int j = i;
...
bool command_match_pi(string& command, unsigned int i, uint32_t& address) {
  if (i == command.length() || command[i] != 'p') return false;
  i++;
  if (i == command.length() || command[i] != 'i') return false;
  i++;
  if (!command_skip_required_whitespace(command, i)) return false;
  if (!command_match_hex_number(command, i, address)) return false;
  command_skip_optional_whitespace(command, i);
...
bool command_match_pd(string& command, unsigned int i, uint32_t& address) {
  if (i == command.length() || command[i] != 'p') return false;
  i++;
  if (i == command.length() || command[i] != 'd') return false;
  i++;
  if (!command_skip_required_whitespace(command, i)) return false;
  if (!command_match_hex_number(command, i, address)) return false;
  command_skip_optional_whitespace(command, i);
...
void interpret_commands(memory* main_memory, processor* cpu, bool verbose) {
  ...
  while (true) {
    ...
    //cout << "address--->"<<dec2hex1(address) << endl;
      }
      else {
	main_memory->set_address(address, data);  // Update memory word
  //cout << "address--->"<<dec2hex1(address)<<"  data---->" << data << endl;
      }
      //cout<<hex << "main-----------hi-----------" << main_memory->read_word(address)<<endl;
   ...
   // cout << " clear point ------------------------------  " <<hex << address << endl;
      }
      else {
	cpu->set_breakpoint(address);  // Set breakpoint at the address
  //cout << " set point ------------------------------  "  <<hex<< address  << endl;
      }
    }
